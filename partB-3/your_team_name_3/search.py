...
from your_team_name_3.util import print_boom, print_move, print_board
from your_team_name_3.pq import PriorityQueue
...
def main():
    ...
    targets = {(x,y): n for n,x,y in data["black"]}
    target_groups = find_explosion_groups(targets.keys())

    # search for an action sequence
    start_state = State(stacks, target_groups, targets)
    start_state = start_state.reverse()
...
class State:
    ...
    """
    def __init__(self, stacks, groups, targets, group_metrics=None, all_targets=None):
        self.stacks = stacks
        self.groups = groups
    ...
    def reverse(self):
        ...
        targets = self.stacks
        target_groups = find_explosion_groups(targets.keys())
        return State(stacks, target_groups, targets)
...
class Move:
    ...
    def apply_to(self, state):
        ...
        if self.b not in new_stacks:
            ...
            new_stacks[self.b] += self.n
        return State(new_stacks, state.groups, state.targets,
...
class Boom:
    ...
    def apply_to(self, state):
        ...
        my_targets = {}
        for target in new_targets.keys():
            my_targets[target] = state.targets[target]
        return State(new_stacks, new_groups, my_targets,
            group_metrics=state.group_metrics,
